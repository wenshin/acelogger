import { SpanContext, TimeInput, TraceFlags, Attributes, SpanKind, HrTime } from '@opentelemetry/api';
import { isTimeInputHrTime, hrTimeToMilliseconds } from '@opentelemetry/core';
import { Tracer, TracerStruct, SpanStruct, SpanOptions } from './api';
...
type SpanContextConfig = { traceId?: string, spanId?: string, traceFlags?: TraceFlags } & SpanContext
...
export default class SimpleTracer implements Tracer {
  ...
  constructor(config: SimpleTracerConfig) {
    ...
    this.startTime = config.startTime && isTimeInputHrTime(config.startTime)
      ? hrTimeToMilliseconds(config.startTime as HrTime)
      : Date.now();
  }
